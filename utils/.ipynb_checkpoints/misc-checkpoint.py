...
def collate_fn(raw_batch):
    ...
    info = raw_batch[5]
    batch = [img_data, text_data, bbox, info]
    return tuple(batch)
    ...
    # bbox = torch.tensor(raw_batch[4])
    # info = raw_batch[5]
    # dsc8_id = torch.tensor(raw_batch[6])
    # dsc8_mask = torch.tensor(raw_batch[7])
    # dsc8_data = []
    # for i in range(8):
    #     dsc_id = dsc8_id[:,i,:]
    #     dsc_mask = dsc8_mask[:,i,:]
    #     dsc_data = NestedTensor(dsc_id, dsc_mask)
    #     dsc8_data.append(dsc_data)
    # batch = [img_data, text_data, bbox, info, dsc8_data]
    # return tuple(batch)
...
#     bbox = torch.tensor(raw_batch[4])
#     info = raw_batch[5]
#     dsc_id = torch.tensor(raw_batch[6])
#     dsc_mask = torch.tensor(raw_batch[7])
#     dsc_data = NestedTensor(dsc_id, dsc_mask)
#     batch = [img_data, text_data, bbox, info, dsc_data]
#     return tuple(batch)
